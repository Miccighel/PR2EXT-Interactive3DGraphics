...
function generateData(rowNumb , colNumb, min, max) {    
    ...
    for(var i=0;i<rowNumb;i++){
        ...
        for (var j=0;j<colNumb;j++) {
            ...
            tempArray.push((Math.random() * (max - min) + min).toFixed(2));
        }
        data.push(tempArray);        
    }
    return data;
        ...
        if(intersects.length>0) {
            ...
            switch(chart['type']){
                case 'pieChart':
                    ...
                    valueLabelGeometry = new THREE.TextGeometry( valueLabelText, {size: 2,height: 0.1,curveSegments:2,font: "helvetiker"});
