...
function generatePieChart(data, visualSet, rowToShow) {
    ...
    for(var i = 0; i < data[rowToShow-1].length;i++){
        ...
        if(visualSet.length == 7) {
            pieMaterial = new THREE.MeshPhongMaterial({ambient: 0x030303, color: visualSet[i], specular: 0xffffff, shininess: 10, shading: THREE.FlatShading } );
        } else {
            // The material is created with the color of the color set assigned to the current row
...
function generateBarChart(data, visualSet, maxValue) {
    ...
    for(var i=0;i<data.length;i++){
        ...
        for(var j=0;j<data[i].length;j++){
            ...
            if(visualSet.length==7) {
                barMaterial = new THREE.MeshPhongMaterial({ambient: 0x030303, color: visualSet[i], specular: 0xffffff, shininess: 10, shading: THREE.FlatShading } );
            } else {
                // The material is created with the color of the color set assigned to the current row
...
function generateAreaChart(data, visualSet, maxValue) {
    ...
    for(var i=0;i<data.length;i++){
        if(visualSet.length == 7) {
            areaMaterial = new THREE.MeshPhongMaterial({ambient: 0x030303, color: visualSet[i], specular: 0xffffff, shininess: 10, shading: THREE.FlatShading } );
        } else {
            // The material is created with the color of the color set assigned to the current row
