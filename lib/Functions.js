    ...
    visualSet.onChange(function(value) {
        switch (params.Visual_Set) {
            ...
            case 'Material Set 1':
                visuals = new Array();
                var materialParams = {
                    map: 'textures/metal1diffuse.jpg',
                    specularMap: 'textures/metal1specular.jpg',
                }
                var planeParams = {
                    map: 'textures/metal2diffuse.jpg',
                    specularMap: 'textures/metal2specular.jpg',
                }
                visuals.push(materialParams);
        ...
        if(intersects.length>0) {
            ...
            switch(chart['type']){
                case 'pieChart':
                    ...
                    valueLabelMaterial = new THREE.MeshBasicMaterial( { color: '#000000', overdraw: true } );
...
function generatePieChart(data, visualSet, rowToShow) {
    ...
    for(var i = 0; i < data[rowToShow-1].length;i++){
        ...
        if(visualSet.length<=1) {
            var materialParams = visualSet[0];
            var map = THREE.ImageUtils.loadTexture(materialParams.map);
            map.wrapS = THREE.RepeatWrapping;
            map.wrapT = THREE.RepeatWrapping;
            var specularMap = THREE.ImageUtils.loadTexture(materialParams.specularMap);
            pieMaterial = new THREE.MeshPhongMaterial({});
        } else {
            // The material is created with the color of the color set assigned to the current row
...
function showPieChart() {
    ...
    rowsNumber.onChange(function(value) {
        ...
        scene = new THREE.Scene();
        generateLights(params.Rows,params.Columns,params.Max_Value,'#FFFFFF',2.0);
    ...
    for(var i=0;i<data.length;i++){
        ...
        for(var j=0;j<data[i].length;j++){
            ...
            if(visualSet.length<=2) {
                map.wrapT = THREE.RepeatWrapping;
                map.wrapS = THREE.RepeatWrapping;
                map.repeat.set( 1, data[i][j]/2 );
                map.anisotropy = renderer.getMaxAnisotropy();

                specularMap.wrapT = THREE.RepeatWrapping;
                specularMap.wrapS = THREE.RepeatWrapping;
                specularMap.repeat.set( 1, data[i][j]/2 );
                specularMap.anisotropy = renderer.getMaxAnisotropy();

                barMaterial = new THREE.MeshPhongMaterial({map:map, specularMap:specularMap});
...
function generateAreaChart(data, visualSet, maxValue) {
    ...
    for(var i=0;i<data.length;i++){
        if(visualSet.length<=2) {
            ...
            specularMap.wrapT = THREE.RepeatWrapping;
            specularMap.wrapS = THREE.RepeatWrapping;
            specularMap.repeat.set( 200, 200);
            specularMap.anisotropy = renderer.getMaxAnisotropy();

            areaMaterial = new THREE.MeshPhongMaterial({map:map,specularMap:specularMap});
        } else {
            // The material is created with the color of the color set assigned to the current row
        ...
        for(var k=0;k<areaGeometry.vertices.length-2;k= k+2){
            ...
            areaGeometry.faceVertexUvs[ 0 ].push( [
...
function showAreaChart() {
    ...
    rowsNumber.onChange(function(value) {
        ...
        data = generateData(params.Rows, params.Columns,1,params.Max_Value);
        scene = new THREE.Scene();
        generateLights(params.Rows,params.Columns,params.Max_Value,'#FFFFFF',1.5);
...
function generateLights(rows,columns,maxValue,color,lightPower) {
    spotLight1 = new THREE.SpotLight(color,lightPower,0.0,Math.PI/3,35.0);
    spotLight1.position.set(rows + 60.0,(maxValue/2) + 5.0,columns);
    spotLight2 = new THREE.SpotLight(color,lightPower,0.0,Math.PI/3,35.0);
    spotLight2.position.set(rows,(maxValue/2)+5.0,columns + 60.0);
    spotLight3 = new THREE.SpotLight(color,lightPower,0.0,Math.PI/3,35.0);
    spotLight3.position.set(rows,(maxValue) + 30.0,columns);

    helper = new THREE.SpotLightHelper(spotLight1,50.0);
    helper2 = new THREE.SpotLightHelper(spotLight2,50.0);
    helper3 = new THREE.SpotLightHelper(spotLight3,50.0);
