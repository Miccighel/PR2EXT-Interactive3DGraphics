...
function generatePieChart(data, visualSet, rowToShow) {
    ...
    for(var i = 0; i < data[rowToShow-1].length;i++){
        ...
        if(visualSet.length<=2) {
            ...
            pieMaterial = new THREE.MeshPhongMaterial({map:map, specularMap:specularMap});
        } else {
            // The material is created with the color of the color set assigned to the current row
        ...
        for(var j=2;j< pieGeometry.vertices.length-2;j= j+2){
            ...
            pieGeometry.faceVertexUvs[ 0 ].push( [
...
function generateLights(rows,columns,maxValue,color,lightPower) {
    ...
    spotLight1.target.position.set(0,(maxValue/2),columns);
    spotLight2 = new THREE.SpotLight(color,lightPower,0.0,Math.PI/3,50.0);
    spotLight2.castShadow = true;
    spotLight2.shadowDarkness = 0.3;
    spotLight2.position.set(rows,(maxValue/2)+5.0,columns + 60.0);
    spotLight2.target.position.set(rows,(maxValue/2),0);
    spotLight3 = new THREE.SpotLight(color,lightPower,0.0,Math.PI/3,50.0);
    spotLight3.castShadow = true;
    spotLight3.shadowDarkness = 0.3;
    spotLight3.position.set(rows,(maxValue) + 30.0,columns);
    spotLight3.target.position.set(rows,0,columns);
