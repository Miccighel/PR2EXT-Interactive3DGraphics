...
function showPieChart() {
    ...
    rowsNumber.onChange(function(value) {
        ...
        scene = new THREE.Scene();
        generateLights(params.Rows,params.Columns,params.Max_Value,'#ABCDEF',1.0);
...
function animatePieChart(chart) {
    ...
    var target = { x : 1, z: 1};
    var tween = new TWEEN.Tween(scale).to(target, 1000);
...
function generateAreaChart(data, visualSet, maxValue) {
    ...
    for(var i=0;i<data.length;i++){
        ...
        areaMaterial = new THREE.MeshBasicMaterial({color:visualSet[i], side:THREE.FrontSide,opacity: 0.65,
...
function generateLights(rows,columns,maxValue, color,lightPower) {
    ...
    spotlight1 = new THREE.SpotLight(0xABCEF,lightPower,0.0,Math.PI/4);
    spotlight1.position.set(rows + 2.0,(maxValue/2) + 2.0,columns/2);
    helper = new THREE.SpotLightHelper(spotlight1,50.0);
    scene.add(helper);
    spotlight2 = new THREE.SpotLight(0x000000,lightPower,0.0,Math.PI/4);
    spotlight2.position.x = rows/2;
    spotlight2.position.z = columns + 2.0;
    spotlight2.position.y = (maxValue/2) + 2.0;
